...
export default function ProductsClient({ initialProducts, initialCategories, staticProducts, initialSelectedCategory = 'Best Seller' }: ProductsClientProps) {
    ...
    const handleCategoryChange = async (catId: string | 'Best Seller') => {
        ...
        try {
            ...
            setProducts(mergeProducts(data, getStaticProducts(staticProducts, catId)));
        } catch (error) {
            console.error('Failed to load products:', error);
            // Fallback: show static products for this category even if API fails
            setProducts(getStaticProducts(staticProducts, catId));
        } finally {
            setLoading(false);
        }
    };
    ...
    const toggleProduct = (product: Product) => {
        setSelectedItems(prev => {
            ...
            if (newItems[product.id]) {
                ...
                trackEvent({ name: 'select_product', productName: product.name, productId: product.id, price: product.price });
            }
            return newItems;
        });
    };
    ...
    const handleSubmit = () => {
        ...
        const items = getSelectedList().map(p => `- ${p.name} x${p.quantity} (₹${p.price})`).join('\n');
        const msg = `Hi! I'd like to order:\n\n${items}\n\nName: ${formData.name}\nAddress: ${formData.address}`;

        trackEvent({ name: 'whatsapp_click', context: 'order_form' });
    ...
    const openProductDetail = (product: Product) => {
        if (product.in_stock) {
            ...
            trackEvent({ name: 'view_product', productName: product.name, productId: product.id, price: product.price });
        }
    };
                                ...
                                >
                                    <div className="product-image-stage relative aspect-[4/5] overflow-hidden">
                                        <Image
                                            ...
                                            className={`product-image object-contain transition-transform duration-500 ${product.in_stock ? 'group-hover:scale-105' : 'grayscale'}`}
                                        ...
                                        {product.in_stock && (
                                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-300 flex items-center justify-center">
                                                <span className="bg-white/90 backdrop-blur-sm text-[#8B0000] px-4 py-2 rounded-full text-xs font-bold opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0">
                                                    View Details
                                                </span>
                                            </div>
                                        )}
                                    ...
                                    <div className="p-3 sm:p-4">
                                        ...
                                        >
                                            <h3 className="font-semibold text-xs sm:text-base mb-1 leading-tight line-clamp-2 h-8 sm:h-12" style={{ color: product.in_stock ? '#2d2d2d' : '#9ca3af' }}>
                                                {product.name}
                                            </h3>
                                        </Link>
            ...
            {totalSelectedCount > 0 && (
                <div className="fixed bottom-24 sm:bottom-24 left-1/2 -translate-x-1/2 z-30 animate-fade-in-up md:hover:scale-110 transition-transform">
                    <button
                        onClick={() => {
                            setShowForm(true);
                            trackEvent({ name: 'begin_order', itemCount: totalSelectedCount });
                        }}
            ...
            {selectedProductDetail && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-2 sm:p-4" onClick={() => setSelectedProductDetail(null)}>
                    ...
                    >
                        ...
                        >
                            <Image
                                src={selectedProductDetail.image_url || '/logo.png'}
                                alt={`${selectedProductDetail.name} - handmade gift by Kalangan`}
                                fill
                        ...
                        <div className="md:w-1/2 flex flex-col flex-1 overflow-hidden">
                            <div className="p-5 sm:p-8 flex-1 overflow-y-auto scrollbar-hide">
                                <div className="space-y-4">
                                    ...
                                    <Link
                                        href={`/products/${generateSlug(selectedProductDetail.name, selectedProductDetail.id)}`}
                                        className="text-sm text-[#8B0000] underline hover:no-underline"
                                    >
